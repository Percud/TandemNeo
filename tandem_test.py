    ...
    with open('species_list.txt', 'r') as list:
        with open('assembly_summary_refseq.txt', 'r') as summary:
            ...
            for v, s in version_specie.items():
                ...
                if not os.path.exists(cwd + '/query/' + specie + '.gtf') and not os.path.exists(cwd + '/query/' + specie + '.faa') and not os.path.exists(cwd + '/sub/' + specie + '.gtf') and not os.path.exists(cwd + '/sub/' + specie + '.faa') and not os.path.exists(cwd + '/' + specie + '.faa') and not os.path.exists(cwd + '/' + specie + '.gtf'):
...
def movefiles():
    ...
    dirf = os.path.join(cwd + '/query/main')
    ...
    for line in queries:
        if not os.path.exists(cwd + '/query/main/' + line + '_main.faa'):
            try:
